...
class LpbInWarehouse extends StatefulWidget {
  ...
  @override
  _LpbInWarehouseState createState() => _LpbInWarehouseState();
}

class _LpbInWarehouseState extends State<LpbInWarehouse> {
      ...
      if (allData != null) {
        final filteredData =
            allData.where((item) {
              ...
              final status =
                  item['status_barang']?.toString().toLowerCase() ?? '';
              return containerId == null && status == 'in warehouse';
            }).toList();
